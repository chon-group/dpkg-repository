...
getLatest(){
    ...
    if [ ! -f $SRC$GITREPO/$FILENAME ]; then
        ...
        echo "The $GITREPO is the latest version, skipping..."
    fi
}


#getLatest DEBIAN-PACKAGE-NAME   GITHUB-REPO-NAME
#getLatest "chonos-lxc-template" "dpkg-chonos-lxc-template"
#getLatest "choncloud" "dpkg-choncloud"
...
#getLatest "jason-embedded" "dpkg-jasonembedded"
...
#getLatest "contextnet-server" "contextNetServer"
#getLatest "chonos-simulide" "dpkg-simulide"
#getLatest "chonos-wsl-installer" "dpkg-chonos-wsl-installer"

mkdir -p public_html/dists/chonos/main/binary-all
cd public_html

dpkg-scanpackages -m chonos/ > dists/chonos/main/binary-all/Release
